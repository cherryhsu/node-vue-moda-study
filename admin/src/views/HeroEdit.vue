...
export default {
  ...
  methods: {
    ...
    async save() {
      let res;
      if (this.id) {
        res = await this.$http.post(`rest/heros/${this.id}`, this.model);
      } else {
    ...
    async fetch() {
      ...
      this.model = Object.assign({}, this.model, res.data); //语法: Object.assign(target, …sources) target: 目标对象,sources: 源对象用于将所有可枚举属性的值从一个或多个源对象复制到目标对象。它将返回目标对象。
    },
